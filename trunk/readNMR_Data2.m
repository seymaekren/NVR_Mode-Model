...
if (useOrigData) 
    ...
    [doNotUseVECTORS,doNotUseTYPES,doNotUseRESNUMS,doNotUseSSTRUCT, doNotUseHBOND, doNotUseALLDISTS,doNotUseIALLDISTS, HSQCDATA] = loaddata(filename);
...
for i=1:size(HSQCDATA,1)
    ...
    for j=1:length(x)
        ...
        if(~isempty(y))%isempty length degistirildi
            NOES(i,y)=1; 
            NOES(y,i)=1;
        end
    end
