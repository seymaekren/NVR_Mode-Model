...
/*
 * Computes the single and pair interval terms in the MinDEE criterion
 * 
...
public class MinDEEIntervals {
	...
	MinDEEIntervals(float arpMatrix[][], float arpMatrixMax[][], int numResInActiveSite, 
			...
			StrandRotamers systemLRot, boolean prunedRotAtRes[], boolean scInt, float dist[][], float maxSc, RotamerLibrary rlP) {
	...
	private double PairInt (int atPos1, int atAA1, int atRot1, int atPos2, int atAA2, int atRot2){
		...
		if ((!eliminatedRotAtPos[index1-1])&&(!eliminatedRotAtPos[index2-1])){
