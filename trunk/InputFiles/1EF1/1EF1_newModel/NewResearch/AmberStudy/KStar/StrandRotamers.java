...
public class StrandRotamers implements Serializable {
	...
	private char curAAType[] = null;    //the single character AA code currently assumed by each residue in the strand
	...
	private void setupStrand(Strand s) {
		...
		curAAType = new char[numberOfResidues];
		...
		for(int i=0;i<numberOfResidues;i++) {
			...
			numAllowable[i] = 0;
			curAAType[i] = rl.getOneCharAACode(s.residue[i].name);
			curRotNum[i] = -1;
		}
	}
	...
	// Returns the current single letter AA code
	//  for the specified residue
	public char getCurRotType(int index){
		return(curAAType[index]);
	}

	// Returns the current rotamer number
	//  for the specified residue
	public int getCurRotNum(int index){
		return(curRotNum[index]);
	}
	...
	public float getCurAllowableVolume(){
		...
		for(int i=0;i<numberOfResidues;i++){
			if(numAllowable[i] > 0){
				...
				if ((curAAType[i] == 'H') || (curAAType[i] == 'h'))
					tmpstg = "HIS";
				else
					tmpstg = rl.getThreeLetAACode(curAAType[i]);
				localAANum = rl.getAARotamerIndex(tmpstg);
	...
	//  COld into CNew
	// In the function call at1 = COld, at2 = CAOld = CANew,
	//  at3 = NOld = NNew, and at4 = CNew
	// The axis is returned in axis and the angle is in thetaAngle
	// This is one of the more obfuscated functions
	...
	//  3) Rotate around N-CA axis to get C's to overlap
	//
	// This funtion is rather complicated due to the bookkeeping
	...
	public void changeResidueType(Molecule m, int resNum, String newResType, boolean addHydrogens, boolean connectResidue, boolean useOldBBatoms) {
		...
		// Rotate Cs to overlap - now the residue backbones should be aligned		
		if ( COld.distance(CNew) > 0.0001 ) {
			getRotationInfoB(COld, CAOld, NOld, CNew, thetaDeg, rotAxis);
			for(int q=0;q<r.numberOfAtoms;q++)
				atomList[q] = q;
		...
		// 	the CAs, Cs, and CBs are already aligned, but may differ due to differences between the template PPR and the residue backbone in the PDB
		if (useOldBBatoms){
		...
		curAAType[resNum] = rl.getOneCharAACode(newResType);
		curRotNum[resNum] = -1;
