...
public class EEF1 implements Serializable {
	...
	private int getSolvGroupIndex(Atom at1){
		...
		if (elementType.equalsIgnoreCase("C")) {
			...
			if ( at1.name.equalsIgnoreCase("CG") && 
					( AAname.equalsIgnoreCase("TYR") || AAname.equalsIgnoreCase("PHE") || AAname.equalsIgnoreCase("HIP") ) )
				return findSolvGroup("CR"); //CG of (TYR or PHE or HIP)
	...
	private boolean isCarboxylO(Atom at1){
		...
		if (!at1.elementType.equalsIgnoreCase("O")) //not O
			...
			if (at1.bond!=null){ //check single bonds
			...
			if (at1.doubleBond!=null){ //check double bonds
				for (int i=0; i<at1.doubleBond.length; i++){ //check the double bonds
					
					Atom at2 = m.atom[at1.doubleBond[i]];
	...
	private boolean isCOhelper(Atom at2){		
		...
		int numBoundO = 0;
		if (at2.bond!=null){ //check single bonds
			for (int j=0; j<at2.bond.length; j++){
				if (m.atom[at2.bond[j]].elementType.equalsIgnoreCase("O"))
					numBoundO++;
			}
		}
		if (at2.doubleBond!=null){ //check double bonds
			for (int j=0; j<at2.doubleBond.length; j++){ //check double bonds
				if (m.atom[at2.doubleBond[j]].elementType.equalsIgnoreCase("O"))
					numBoundO++;
			}
		}
	...
	private boolean isAromatic(Atom at1){
		...
		if ( !(AAname.equalsIgnoreCase("PHE") || AAname.equalsIgnoreCase("TYR") 
					|| AAname.equalsIgnoreCase("TRP") || AAname.equalsIgnoreCase("HIP") ) ) //not PHE and not TYR and not TRP or HIS, so not aromatic
				return false;
		else {//PHE or TYR or TRP or HIP, so check if aromatic atom
			
			if (!at1.elementType.equalsIgnoreCase("C")){ //not a carbon
				if ( at1.elementType.equalsIgnoreCase("N") && (!at1.name.equalsIgnoreCase("N")
						&& (AAname.equalsIgnoreCase("TRP") || AAname.equalsIgnoreCase("HIP")) ) ) //N in the ring of TRP or HIP
					return true;
				else
					return false;
			}
