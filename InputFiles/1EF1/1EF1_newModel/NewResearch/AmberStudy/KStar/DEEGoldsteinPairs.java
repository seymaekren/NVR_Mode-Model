...
public class DEEGoldsteinPairs {
	...
	DEEGoldsteinPairs(float arpMatrix[][], float arpMatrixMax[][], int numResInActiveSite, 
			...
			boolean doMin, boolean spFlags[][], boolean useSF, boolean mb, boolean dDEE, boolean minBB, boolean scaleInt, float maxSc, RotamerLibrary rlP) {
		...
		if (!scaleInt) //no sclaing of the interval terms performed
			maxScale = 1.0f;
		else
			maxScale = maxSc;
	}
	...
	private boolean CanEliminate (int posNum1, int AANumAtPos1, int rotNumAtPos1, int posNum2, 
		...
		index_r1 = 1 + posNum1*numTotalRot + rotIndOffset[AANumAtPos1] + rotNumAtPos1;
		index_r2 = 1 + posNum2*numTotalRot + rotIndOffset[AANumAtPos2] + rotNumAtPos2;
		minIndVoxelE = pairwiseMinEnergyMatrix[index_r1][0] + pairwiseMinEnergyMatrix[index_r2][0]; //formula term 1
		minShellResE = pairwiseMinEnergyMatrix[0][index_r1] + pairwiseMinEnergyMatrix[0][index_r2];
		minPairE = pairwiseMinEnergyMatrix[index_r1][index_r2];
		
		
		//if ((minIndVoxelE<=stericEThreshIntra)&&(minShellResE<=stericEThreshPair)){//check only if not an unallowed steric
		if ((!eliminatedRotAtPos[index_r1-1])&&(!eliminatedRotAtPos[index_r2-1])
				&&(!splitFlags[index_r1-1][index_r2-1])){ //not pruned
			...
			for (int AA1=0; AA1<numAAtypes[posNum1]; AA1++){
				...
				for (int altRot1=0; altRot1<numRotForAAatPos1; altRot1++){
					...
					if (!((altAA1==AANumAtPos1)&&(altRot1==rotNumAtPos1))){
						...
						index_t1 = 1 + posNum1*numTotalRot + rotIndOffset[altAA1] + altRot1;
						
						if ((!eliminatedRotAtPos[index_t1-1])){ //not pruned
							...
							for (int AA2=0; AA2<numAAtypes[posNum2]; AA2++){
								...
								for (int altRot2=0; altRot2<numRotForAAatPos2; altRot2++){
									...
									if (!((altAA2==AANumAtPos2)&&(altRot2==rotNumAtPos2))){
										...
										index_t2 = 1 + posNum2*numTotalRot + rotIndOffset[altAA2] + altRot2;
										
										if ((!eliminatedRotAtPos[index_t2-1])){ //not pruned 
							
											maxIndVoxelE = pairwiseMaxEnergyMatrix[index_t1][0] + pairwiseMaxEnergyMatrix[index_t2][0];		//formula term 2
											maxShellResE = pairwiseMaxEnergyMatrix[0][index_t1] + pairwiseMaxEnergyMatrix[0][index_t2];	
											maxPairE = pairwiseMaxEnergyMatrix[index_t1][index_t2];
	...
	private double IndMinDiffPVE (int firstPos, int firstAA1, int firstRot1, int firstAltAA1, int firstAltRot1, 
		...
		//r at i
		index_r1 = 1 + firstPos*numTotalRot + rotIndOffset[firstAA1] + firstRot1;
		index_r2 = 1 + secondPos*numTotalRot + rotIndOffset[secondAA1] + secondRot1;
		
		//t at i
		index_t1 = 1 + firstPos*numTotalRot + rotIndOffset[firstAltAA1] + firstAltRot1;
		index_t2 = 1 + secondPos*numTotalRot + rotIndOffset[secondAltAA1] + secondAltRot1;
		
		boolean found = false;
		
		if (((!eliminatedRotAtPos[index_r1-1])&&(!eliminatedRotAtPos[index_r2-1]))&&
				((!eliminatedRotAtPos[index_t1-1])&&(!eliminatedRotAtPos[index_t2-1]))){ //not pruned
			...
			for (int AA=0; AA<numAAtypes[thirdPos]; AA++){
				...
				for (int curRot=0; curRot<numRotForAAatPos; curRot++){
					...
					//s at j
					index2 = 1 + thirdPos*numTotalRot + rotIndOffset[curAA] + curRot;
					
					if ((!eliminatedRotAtPos[index2-1])){ //not pruned 
						
						if ((!useFlags)||((!splitFlags[index_r1-1][index2-1])&&(!splitFlags[index_r2-1][index2-1]))){ //not using split flags or not flagged
		
							curEmin = pairwiseMinEnergyMatrix[index_r1][index2] + pairwiseMinEnergyMatrix[index_r2][index2];
							curEmax = pairwiseMaxEnergyMatrix[index_t1][index2] + pairwiseMaxEnergyMatrix[index_t2][index2];
	...
	private double IndInt (int atPos, int atAA, int atRot){
		...
		if ((!eliminatedRotAtPos[index1-1])){ //not pruned 
			...
			double maxE = pairwiseMaxEnergyMatrix[index1][0];
			double minE = pairwiseMinEnergyMatrix[index1][0];
			
			double maxShell = pairwiseMaxEnergyMatrix[0][index1];
	...
	private double PairInt (int atPos1, int atAA1, int atRot1, int atPos2, int atAA2, int atRot2){
		...
		if (((!eliminatedRotAtPos[index1-1]))&&
				((!eliminatedRotAtPos[index2-1]))){ //not pruned 
		
			double maxE = pairwiseMaxEnergyMatrix[index1][index2];
	...
	private boolean CanEliminateLig (int curLigRot,int posNum2, int AANumAtPos2, int rotNumAtPos2){
		...
		//In the energy matrix, column 0 gives the individual energies for each r at i;
		//skip row 0, as the individual energies start from row 1 (and are all in column 0)
		index_r1 = 1 + numSiteResidues*numTotalRot + curLigRot;
		index_r2 = 1 + posNum2*numTotalRot + rotIndOffset[AANumAtPos2] + rotNumAtPos2;
		minIndVoxelE = pairwiseMinEnergyMatrix[index_r1][0] + pairwiseMinEnergyMatrix[index_r2][0]; //formula term 1
		minShellResE = pairwiseMinEnergyMatrix[0][index_r1] + pairwiseMinEnergyMatrix[0][index_r2];
		minPairE = pairwiseMinEnergyMatrix[index_r1][index_r2];
		...
		if ((!eliminatedRotAtPos[index_r1-1])&&(!eliminatedRotAtPos[index_r2-1])
			...
			for (int altRot=0; altRot<numLigRot; altRot++){
				...
				if (curLigRot!=altRot){
					...
					index_t1 = 1 + numSiteResidues*numTotalRot + altRot;
	...
	private double IndMinDiffPVELig (int firstRot1, int firstAltRot1, int secondPos, int secondAA1, int secondRot1, 
		...
		//r at i
		index_r1 = 1 + numSiteResidues*numTotalRot + firstRot1;
		index_r2 = 1 + secondPos*numTotalRot + rotIndOffset[secondAA1] + secondRot1;
		
		//t at i
		index_t1 = 1 + numSiteResidues*numTotalRot + firstAltRot1;
		index_t2 = 1 + secondPos*numTotalRot + rotIndOffset[secondAltAA1] + secondAltRot1;
