...
public class Atom implements Serializable {
	...
	int	bond[], doubleBond[], tripleBond[];		// arrays of indices of single, double, and triple bonded atoms
	int	elementNumber=0;					// element number
	int	numberOfConnections=0;		// the number of single, double, and triple bonds
	int	numberOfBonds=0;					// the number of single bonds
	...
	// Adds a doubleBond from this atom to atom number j
	// If we're in a molecule then j should be molecule based
	public void addDoubleBond(int j){
		if (doubleBond==null){
			...
			// If we already have a double bond to this atom we're done
			if (doubleBondedTo(j))
				return;
	...
	// Adds a tripleBond from this atom to atom number j
	// If we're in a molecule then j should be molecule based
	public void addTripleBond(int j){
		if (tripleBond==null){
			...
			// If we already have a triple bond to this atom we're done
			if (tripleBondedTo(j))
				return;
	...
	// Deletes the jth doubleBond of this atom
	// *NOTE j is not an atom number, it is an index into the
	//  doubleBond array
	public void deleteDoubleBond(int j){
		if (numberOfDoubleBonds==0)
			return;
	...
	// Deletes the jth tripleBond of this atom
	// *NOTE j is not an atom number, it is an index into the
	//  tripleBond array
	public void deleteTripleBond(int j){
		if (numberOfTripleBonds==0)
			return;
	...
	// Returns true/false, is this atom doubleBonded to atom j
	...
	// Returns true/false, is this atom tripleBonded to atom j
