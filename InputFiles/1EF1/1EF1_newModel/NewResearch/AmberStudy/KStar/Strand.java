      ...
      System.arraycopy(residue, residueNumber+1, smallerResidueArray,
		...
		for(int i=0; i<numberOfResidues-1; i++){
			for(int j=0; j<residue[i].numberOfAtoms; j++){
				...
				for(int m=0; m<residue[i].atom[j].numberOfDoubleBonds; m++){
					if (residue[i].atom[j].doubleBond[m] >= lowAtomIndex){
						if (residue[i].atom[j].doubleBond[m] <= highAtomIndex){
							residue[i].atom[j].deleteDoubleBond(m);
							m--;
						}
						else
							residue[i].atom[j].doubleBond[m] -= oldResNumAtoms;
					}
				}
				for(int m=0; m<residue[i].atom[j].numberOfTripleBonds; m++){
					if (residue[i].atom[j].tripleBond[m] >= lowAtomIndex){
						if (residue[i].atom[j].tripleBond[m] <= highAtomIndex){
							residue[i].atom[j].deleteTripleBond(m);
							m--;
						}
						else
							residue[i].atom[j].tripleBond[m] -= oldResNumAtoms;
					}
				}
			}
		}
		...
		for(int i=0; i<numberOfResidues; i++){
			if (!(residue[i].strandResidueNumber==residueNumber)){
				for(int j=0; j<residue[i].numberOfAtoms; j++){
					...
					for(int m=0; m<residue[i].atom[i].numberOfDoubleBonds; m++){
						if (residue[i].atom[i].doubleBond[m] == moleculeAtomNumber){
							residue[i].atom[i].deleteDoubleBond(m);
							m--;
						}
						else if (residue[i].atom[i].doubleBond[m] > moleculeAtomNumber){
							residue[i].atom[i].doubleBond[m] -= 1;
						}
					}
					for(int m=0; m<residue[i].atom[i].numberOfTripleBonds; m++){
						if (residue[i].atom[i].tripleBond[m] == moleculeAtomNumber){
							residue[i].atom[i].deleteTripleBond(m);
							m--;
						}
						else if (residue[i].atom[i].tripleBond[m] > moleculeAtomNumber){
							residue[i].atom[i].tripleBond[m] -= 1;
						}
					}
				}
			}
